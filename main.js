(function () {
  ...
  function getFormUrl() {
    return String(SITE.googleFormUrl || "").trim();
  }
  ...
  function refreshFormAccessFromSiteConfig() {
    const formUrl = getFormUrl();
    const iframe = document.querySelector(".form-embed");
    const fallbackBtn = document.querySelector(".cta-secondary");
    const printAccessLink = document.getElementById("printFormUrl");
    const qrImg = document.getElementById("printFormQr");

    if (iframe) {
      const nextEmbedUrl = toEmbedFormUrl(formUrl);
      if ((iframe.getAttribute("src") || "") !== nextEmbedUrl) {
        iframe.src = nextEmbedUrl;
      }
    }
    if (fallbackBtn) {
      fallbackBtn.href = formUrl || "#";
    }
    if (printAccessLink) {
    ...
    if (qrImg) {
      if (formUrl) {
        const matrix = window.BazarQR && window.BazarQR.createQrMatrixV3L ? window.BazarQR.createQrMatrixV3L(formUrl) : null;
        const qrDataUrl = window.BazarQR && window.BazarQR.qrMatrixToPngDataUrl ? window.BazarQR.qrMatrixToPngDataUrl(matrix) : "";
        if (qrDataUrl) {
          qrImg.src = qrDataUrl;
          qrImg.style.display = "";
        } else {
  ...
  function buildSections() {
    ...
    STALLS.forEach((stall, index) => {
      ...
      const section = document.createElement("stall-slide");
      section.model = {
        index,
        stall,
        profile,
        sectionTarget,
        mobileStopSide: mobileStopMeta.side,
    ...
    iframe.addEventListener("load", () => {
      frameWrap.classList.remove("is-loading", "is-slow", "is-error");
    }, { once: true });
    iframe.addEventListener("error", () => {
      frameWrap.classList.remove("is-loading", "is-slow");
      frameWrap.classList.add("is-error");
    }, { once: true });
    window.setTimeout(() => {
      if (frameWrap.classList.contains("is-loading")) {
        frameWrap.classList.add("is-slow");
      }
    }, 5000);
    iframe.referrerPolicy = "no-referrer-when-downgrade";
    iframe.src = toEmbedFormUrl(getFormUrl());
    ...
    printAccessLink.id = "printFormUrl";
    printAccessLink.href = formUrl || "#";
  ...
  function lerp(a, b, t) {
    if (window.BazarMotion && window.BazarMotion.lerp) return window.BazarMotion.lerp(a, b, t);
    return a + (b - a) * t;
  }

  function easeOutBack(t) {
    if (window.BazarMotion && window.BazarMotion.easeOutBack) return window.BazarMotion.easeOutBack(t);
    const c1 = 1.70158;
    const c3 = c1 + 1;
  ...
  function easeInOutCubic(t) {
    if (window.BazarMotion && window.BazarMotion.easeInOutCubic) return window.BazarMotion.easeInOutCubic(t);
    return t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2;
  }

  function easeOutCubic(t) {
    if (window.BazarMotion && window.BazarMotion.easeOutCubic) return window.BazarMotion.easeOutCubic(t);
    return 1 - Math.pow(1 - t, 3);
  }

  function smoothToward(current, target, speed, dtSec) {
    if (window.BazarMotion && window.BazarMotion.smoothToward)
      return window.BazarMotion.smoothToward(current, target, speed, dtSec);
  ...
  function initShareAndPdfActions() {
    ...
    if (window.BazarShare && window.BazarShare.initShareButtons) {
      window.BazarShare.initShareButtons({ baseUrl, text: shareTextRaw });
    }
    ...
    if (pdfBtn) {
      pdfBtn.addEventListener("click", async () => {
        ...
        try {
          if (!window.BazarPDF || !window.BazarPDF.exportAndPreview) {
            throw new Error("Module PDF belum termuat.");
          }
          await window.BazarPDF.exportAndPreview({
            sectionSelector: ".festival-section",
            captureBodyClass: "pdf-export",
            beforeCapture: refreshFormAccessFromSiteConfig,
            formUrl: getFormUrl(),
            headerTitle: SITE.title,
            headerSubtitle: SITE.subtitle,
            gerobakSrc: (document.querySelector("#gerobak img") || {}).src || ""
          });
        } catch (error) {
          console.error("PDF export error:", error);
          const msg = error && error.message ? error.message : "Unknown error";
          window.alert(`Gagal membuat PDF: ${msg}`);
        } finally {
  ...
  async function initApp() {
    ...
    buildSections();
    refreshFormAccessFromSiteConfig();
